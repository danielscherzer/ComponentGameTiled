...
namespace Example.Core
{
	internal partial class TileRenderer
	{
		...
		//internal TileSet GetTileSet(uint gid)
		//{
		//	if(tiles.TryGetValue(gid, out var tileData))
		//	{
		...
		{
			var data = new TileData { TileSet = tileSet, X = x, Y = y };
		...
		private struct TileData
		{
			...
			public Box2 TexCoords => Box2Extensions.CreateFromMinSize(X, Y, TileSet.TileWidth, TileSet.TileHeight);

			public override string ToString() => $"Position={X},{Y} TileSet={TileSet.ImageName}";
		}

		private readonly Dictionary<uint, Animation> _animations = new();
		private readonly Dictionary<uint, TileData> _tiles = new();
